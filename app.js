...
let sb=null,currentUser=null,currentProfile=null,jobs=[],currentPhotos=[],wizardStep=0,currentPhotoGroup="before",completedJob=null,cameraStream=null,cameraFacing="environment";
...
async function nextStep(){
  ...
  if(wizardStep<4){
    // Before entering Photos, save the job so camera/gallery always attach
    // to a real job even if Android interrupts the browser.
    if(wizardStep===2){
...
async function uploadBlob(blob, filename){
  ...
  if(u.error){
    msg("jobMessage",u.error.message);
    return false;
  }

  const {data:{publicUrl}}=sb.storage.from("job-photos").getPublicUrl(path);
  const ins=await sb.from("job_photos").insert({
    job_id:id,
    storage_path:path,
  ...
  if(ins.error){
    msg("jobMessage",ins.error.message);
    return false;
  }
  ...
  currentPhotos=data||[];
  renderPhotos();
  msg("jobMessage",`${currentPhotoGroup==="before"?"Before":"After"} photo added.`,true);
  return true;
}
...
async function openCamera(){
  ...
  $("cameraTitle").textContent=`Take ${currentPhotoGroup==="before"?"Before":"After"} Photo`;
  ...
  try{
    ...
    msg("jobMessage","Camera could not open: "+text);
    alert("Camera could not open. You can still use Choose Photos. "+text);
  }
}
...
async function capturePhoto(){
  ...
  if(!video.videoWidth||!video.videoHeight){
    msg("cameraMessage","Camera is not ready yet.");
    return;
  }
  ...
  if(!blob){
    msg("cameraMessage","Could not capture photo.");
    return;
  }
  ...
  const ok=await uploadBlob(
    blob,
    `${currentPhotoGroup}-${Date.now()}.jpg`
  );
  ...
  if(ok){
    ...
    // Re-open / keep the jobsheet visible after capture.
    if(!$("jobDialog").open){
      try{$("jobDialog").showModal()}catch(_){}
    }
  }
}
...
async function uploadFiles(files){
  ...
  for(const file of files){
    await uploadBlob(file,file.name||`${currentPhotoGroup}-${Date.now()}.jpg`);
  }
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  $("switchCameraBtn").addEventListener("click",flipCamera);
