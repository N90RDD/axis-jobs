...
async function ensureJobSavedForPhotos(){
  let jobId=$("jobId").value;
  ...
  if(result.error){
    showMessage("jobMessage","Save the basic job details before adding photos: "+result.error.message);
    return null;
  }

  $("jobId").value=result.data.id;
  $("formTitle").textContent=`Edit ${result.data.job_no}`;
  await logAudit(result.data.id,"created");
  await loadJobs();
...
async function uploadFiles(files){
  ...
  const jobId=await ensureJobSavedForPhotos();
  if(!jobId) return;

  showMessage("jobMessage","Uploading photo…",true);
  ...
  for(const file of files){
    ...
    if(upload.error){
      showMessage("jobMessage","Photo failed to upload: "+upload.error.message);
      continue;
    }
  ...
  currentPhotos=data || [];
  renderPhotoPreview();
  showMessage("jobMessage","Photo added to job.",true);
...
$("takePhotoBtn").addEventListener("click",async()=>{
  const jobId=await ensureJobSavedForPhotos();
  if(jobId) $("cameraPhotos").click();
});

$("uploadPhotoBtn").addEventListener("click",async()=>{
  const jobId=await ensureJobSavedForPhotos();
  if(jobId) $("galleryPhotos").click();
});
